 ...
 * 
 * date:			2017/11/08
 * description:		从销售订单携带芯体刻字信息
 * DESCRIPTION:		增加计划完工日期的星期几字段(FPLANFINISHWEEKDAY)
 * DESCRIPTION:		FOR INSERT  只能触发一部分操作,所以用 FOR  UPDATE
 * AUTHOR:			RenBo
 * 
date:			2017/06/27
...
--	, 
	...
	--	, a.FHeadSelfJ0192='1'  
		,a.FHeadSelfJ01101 = c.FHeadSelfS0150						-- 2017-11-08	renbo
		,a.FHeadSelfJ01102 = datename(weekday,A.fplanfinishdate)	-- 2017-11-08	renbo
from			icmo			a 
...
--- merge whyb_sxh_icmoupdate  start---------------------
	 if exists( select 1 from icmo	a   
		join icbom	b on a.fbominterid = b.finterid  and a.fheadselfj0184 = @FGZL and b.fversion = '自动装配'
		)
		begin
		update ICMO set FHeadSelfJ01100= '自动装配' where fheadselfj0184 = @FGZL 
		end
	...
	BEGIN  
			UPDATE C 
			 SET C.FPlanStartDate=
			  CASE WHEN E.FID IN('7ZDZP','7ZPXT','7ZPJJ','7DHZJ','7LQJC','7QHLH') THEN A.FPlanCommitDate 
...
update ICMO set FHeadSelfJ0185=null
from ICMO t1 
inner join inserted t2 on t1.FInterID=t2.FInterID
where t1.FAuxStockQty=0 and t1.FHeadSelfJ0185 is not null
