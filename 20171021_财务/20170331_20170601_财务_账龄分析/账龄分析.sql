alter PROCEDURE AccountAgeAnalyze
...
while @check > 50000
begin
raiserror('查询范围已经超过50000种物料，将会影响到系统性,请缩小物料范围',18,1) 
return
end
...
t41.inv_qty,t41.amt,(case when isnull(t41.inv_qty,0) = 0 then 0 else round(t41.amt/t41.inv_qty,4) end ) as funitprice,
 ...
 (case t43.unpackagedBomUseStatus when 1072 then ''使用'' when 1073 then  ''未使用'' else  '' '' end) as UPBomUseStatus,
 ...
 (case t43.productBomUseStatus when 1072 then ''使用'' when 1073 then ''未使用'' else  '' '' end) as ProdBomUseStatus
  , t44.SaleQty
from		t_ICItem			t40
