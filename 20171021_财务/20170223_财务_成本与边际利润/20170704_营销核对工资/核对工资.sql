...
,	@FDATE1			DATETIME,
	@FDATE2		DATETIME;
	SET @FDATE1 = CAST('2017-06-01' AS DATETIME);
	SET @FDATE2 = CAST('2017-06-30'   AS DATETIME);
	SET @FYear='2017';
		...
		isnull(FAmtNewDev,0) as FAmtNewDev
			  ...
			  , (case when FXiShu=0 then case when (isnull(FAmtsale,0)+isnull(FAmtNewSaler,0))>= (case when FZhiWuCd= 1045 then 350000 
																								   else 150000 end) 
											  then 2000 
											  else (isnull(FAmtsale,0)+isnull(FAmtNewSaler,0))/(case when FZhiWuCd= 1045 then 350000 
																								 else 150000 end)*2000 
											  end
					 else 0 end) as FBaseWages
			  , (case when (isnull(FAmtsale,0)+isnull(FAmtNewSaler,0))<(case when FZhiWuCd = 1045	then 350000 else 150000 end) then 0
					  else ((isnull(FAmtsale,0)+isnull(FAmtNewSaler,0))-(case when FZhiWuCd = 1045  then 350000 else 150000 end))*0.13
						 * (case when isnull(FAmtPlan,0)=0  then 0 
						         else	case	when (isnull(FAmtsale,0)+isnull(FAmtNewSaler,0))/FAmtPlan >=0.9 then 1 
												when (isnull(FAmtsale,0)+isnull(FAmtNewSaler,0))/FAmtPlan >=0.8 and (isnull(FAmtsale,0)+isnull(FAmtNewSaler,0))/FAmtPlan<0.9 then 0.8
												when (isnull(FAmtsale,0)+isnull(FAmtNewSaler,0))/FAmtPlan >=0.7 and (isnull(FAmtsale,0)+isnull(FAmtNewSaler,0))/FAmtPlan<0.8 then 0.6
 ...
 SELECT YEAR(@FDATE1),SUBSTRING('20170601',5,2),0,a.FsalerID ,a.FNumber ,a.FName 
	...
	/*	DROP TABLE #SalerAmt
		...
		SELECT b.fsalerid,b.fname,x.fid, B.FPayAmount20,B.FPayAmount30,B.FPayAmount40,y.fbillno
			,b.finvoicebill,A.FAmtFact1 ,A.FAmtFactNewSaler, A.FAmtFactNewDev,B.FPayAmount10
				  
			FROM #RceiveAmt1 A
			LEFT OUTER JOIN (
			select (CASE WHEN C.FItemID IN(256,200152) THEN 256 ELSE C.FItemID END ) AS FsalerID ,a.finterid as finvoiceid
			,a.fbillid as finvoicebill, (CASE WHEN C.FItemID IN(256,200152) THEN '吴丽艳' ELSE c.fname END ) as fname
				 , (a.FPayAmount10) as FPayAmount10, (a.FPayAmount20) as FPayAmount20
				 , (a.FPayAmount30) as FPayAmount30, (a.FPayAmount40) as FPayAmount40 
			from #ReceiveAmtResult a
			inner join t_Organization b on a.FCustomerID =b.FItemID 
			left outer join t_Emp c on c.FItemID =b.Femployee	and (c.fitemid in (256,200152)	 or c.fname like '李会'	)) B ON A.FsalerID =B.FsalerID 
			left join t_RP_NewCheckInfo  x on x.finterid = b.finvoiceid
			left join icsale y on y.finterid = x.fbillid and x.ftype = 3 --对应销售发票号
			where B.FPayAmount20 <> 0 or B.FPayAmount30  <> 0 or B.FPayAmount40 <> 0
			ORDER BY A.FsalerID
		 ---a.type 5 收款单 a.type 3 发票; b.type 5 销售发票 finvoiceid 发票内码 目前没有使用
		 select  a.ftype,b.FInvoiceID,* FROM t_RP_NewCheckInfo A  INNER JOIN t_RP_Contact  b on/* a.FType =5 and*/  b.FBillID =a.FBillID
where a.fid = 3124;
