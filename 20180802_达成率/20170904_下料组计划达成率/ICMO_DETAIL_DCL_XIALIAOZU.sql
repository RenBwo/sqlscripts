...
end ))//*convert(decimal(10,3),row_number() over( order by a.fplancommitdate,a.fbillno ))*/(case (case 
when  datediff(hour,c.fendworkdate,dateadd(hour ,34,dateadd(day,0,datediff(day,0,a.fplancommitdate))))>=0 then 1 else 0 
end )
when 1 then
row_number() over(partition by (case 
...
/*display only finished workerorder*/
select * from #t1 where finishstatus = 1 
END
