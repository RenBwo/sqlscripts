/*
 * DESCRIPTIONS:	生产任务单明细及达成率查询--四科(stockid=245481)
 ...
 * AUTHOR：			RENBO
 */
alter PROCEDURE [dbo].[ICMO_DETAIL_DCL_4K] 
	@FPlanFDate1	datetime,								--the Plan-Start Date 
	@FPlanFDate2	datetime,								--the Plan-End Date
...
BEGIN
	SET NOCOUNT ON 
	/*检查领料单的工作令号和生产任务单的工作令号是否一致，不一致时，修改领料单的工作令号*/
...
set @fcn=0
/* 
 * select * from t_stock where fname like '%四科%' and fdeleted = 0  --245481
 * select fnumber,* from t_Department where fname like '%四科%' and fdeleted=0
 */
set @stockid = 245481
...
--temptable 92 ，the total in-self-stock-qty per item
	...
	WHERE B.FTranType=2 and c.FItemID = @stockid 
