 ...
 */
-----------客户价格体系 不显示利润总额、利润率、汇率、运费(不含税)、标准成本出厂价、标准成本FOB青岛----------
...
SELECT C.FNumber AS 客户代码, c.FName as 客户名称, I.FMapNumber as 客户对应物料代码
 ...
 left join #stdTranPri rb03 on rb03.trantype = a.fpricetype
  ...
  WHERE 
  	c.fnumber like '%@FCusNo@%'
  	AND B.FModel LIKE '%'+ '@@ItemNumber@@'+'%'
  	AND B.Fmodel LIKE '%'+ '@FItemModel@'+'%'
 	AND isnull(B.F_161,'') LIKE '%'+ '@f-161@'+'%'
 	AND isnull(B.F_146,'') LIKE '%'+ '@f-146@'+'%'
 	AND isnull(B.F_131,'') LIKE '%'+ '@f-131@'+'%'
  	AND isnull(B.F_132,'') LIKE '%'+ '@f-132@'+'%'
   	AND isnull(B.F_158,'') LIKE '%'+ '@f-158@'+'%' 
    AND isnull(B.F_161,'') LIKE '%'+ '@nianfen@'+'%' 
  	AND isnull(I.FMapNumber,'') LIKE '%'+ '@FMapNumber@'+'%' 
   	AND isnull(j.FName,'') LIKE '%'+ '@jfname@'+'%'
 ...
 left join #stdTranPri rb03 on rb03.trantype = a.fpricetype
  ...
  WHERE 
	...
	order by ww.币别代码,ww.利润率,ww.物料代码,ww.客户代码
   
   select @rowcount=count(*) from #temp
   select convert(varchar(20),row_number() over(order by 币别代码,利润率,物料代码,客户代码) )+'/'
   +convert(varchar(20),@rowcount)as sn
   	  	,客户代码 ,客户名称 ,客户对应物料代码
	,客户对应物料名称 ,物料代码
	,物料名称 ,规格型号 ,适用车型 ,适用车型英文 ,辅助属性
	,计量单位代码 ,计量单位
    ,"OE代码" ,"DPI代码" ,"NISSENS代码" ,产品结构形式 
    ,"芯体颜色标准" ,"销货量(从)" ,"销货量(到)"
    ,币别代码 ,币别
    ,价格类型代码 ,价格类型
    ,报价 ,最低限价币别 ,最低限价金额
	...
	,"质量赔偿金%批量事故除外"
	,"返点%"
	,(case 标准成本出厂价 when 0 then '无' else convert(varchar(30),标准成本出厂价) end ) as 标准成本出厂价
	,(case "运费(不含税)" when -1 then '无基础资料' 
	 when 0 then '无' else convert(varchar(30),"运费(不含税)")end ) as '运费(不含税)'
	,(case  "标准成本FOB青岛" when 0 then '无' else convert(varchar(30),"标准成本FOB青岛") end) 
	as '标准成本FOB青岛'
	,"预算汇率"
	,"利润总额"
	,"利润率%"		 	
