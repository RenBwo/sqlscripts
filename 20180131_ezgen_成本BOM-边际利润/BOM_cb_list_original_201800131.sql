...
-- Description:	<成本BOM 查询话面 根据SQL追踪的结果结合友邦实际需求进行修改 原ERP中的成本BOM查询不用了>
...
-- date:		2017/02/23
-- =============================================
CREATE PROCEDURE [dbo].[BOM_cb_list_original_201800131]
	@FName				NVARCHAR(255),
	@FCusno				NVARCHAR(255),
	@FQufn				NVARCHAR(1),
	@FParentItemid		INT,
...
UPDATE #BomCost
SET FPlanPrice=a.FPrice,FPlanAmount=A.FPrice *FQty		-- 20170223
from (
	SELECT ren.FItemID as fitemid , max(ren.FPrice) as FPrice
	 from 
	 (select a.fitemid 
	 ,(case a.fcyid when 1000 then round(isnull(a.fprice,0)*@fexchangerrate*100/(isnull(b.fvalueaddrate,0) + 100) ,2)
	 else round(isnull(a.fprice,0)*100/(isnull(b.fvalueaddrate,0) + 100),2) end) as fprice
	 from t_SupplyEntry a join t_supplier b on a.fsupid = b.fitemid  -- 20170223
	 where a.FDisableDate>=GETDATE() 
	) ren group by ren.FItemID 
	) a
