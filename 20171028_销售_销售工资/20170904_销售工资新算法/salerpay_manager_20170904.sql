 ...
 * Author:			BY RenBwo
 * DATE:			2017/09/04
 * DESCRIPITON:		变更前：只区分内贸业务员与外贸业务员进行计算
 * DESCRIPTION：		变更后：增加了“市场开发科主管”finterid 41470，计算方法同外贸业务员
 * 
...
BEGIN
	...
	BEGIN
		...
		-- 5.6 F 销售提成金额:	外贸以35万为基础；内贸以15万为基础；若销售额<35万 then 0  销售额>=35万 THEN (销售额-35万)*0.13*提成计算系数/100；
		...
		left outer join 
			...
			/*	select FEmpID ,SUM(FConsignAmount) AS Amount_cnt,0 as xishu, MAX(FInterID ) as zhiwucd
				from(
					...
					INNER JOIN t_Organization F ON F.FItemID = A.FSupplyID 
					WHERE A.FTranType=21 and 1=(case when f.F_105 >0 and a.FCheckDate between f.f_106 and f.f_107 then 0 else 1 end)
					  AND A.FDate  BETWEEN @FDATE1 AND @FDATE2 AND A.FStatus =1
			*/	
					SELECT (case when A.FEmpID IN(200152,256) then 256  when A.FEmpID IN(203160,255669) then 255669 else a.FEmpID end) as FEmpID , 
					...
					WHERE( A.FTranType=80 or a.ftrantype = 86 )and 1=(case when f.F_105 >0 and a.FDate between f.f_106 and f.f_107 then 0 else 1 end)
					  ...
					  --modified by renbwo on 2017-01-18
					   ) a
				group by FEmpID		
        ...
        /*		select FEmpID ,SUM(FConsignAmount) AS Amount_cnt,0 as xishu, MAX(FInterID ) as zhiwucd
				from(
					...
					WHERE A.FTranType=21 and 1=(case when f.F_105 >0 and a.FCheckDate between f.f_106 and f.f_107 then 0 else 1 end)
				    AND A.FDate  BETWEEN @FDATE1 AND @FDATE2 AND A.FStatus =1
					and  A.FEmpID IN(200152,256,203160,255669)
			*/
					SELECT (case --when A.FEmpID IN(200152,256) then 200152  
								 when A.FEmpID IN(203160,255669) then 203160 else a.FEmpID end) as FEmpID , 
								 ( case a.ftrantype when 86  then b.fstdamount when 80  then b.fstdamountincludetax  end)  as fstdamountincludetax,--含税金额
		...
		left outer join 
			(select FEmpID ,SUM(fstdamountincludetax) AS Amount_cnt2
				from(
			/* select FEmpID ,SUM(FConsignAmount) AS Amount_cnt2
				from(
						SELECT c.FItemID as FEmpID , B.FConsignAmount,a.FBillNo 
					 FROM ICStockBill A
					INNER JOIN ICStockBillEntry B ON A.FInterID =B.FInterID 
					INNER JOIN t_Organization F ON F.FItemID = A.FSupplyID 
					INNER JOIN t_Emp C ON C.FItemID=f.Femployee
					WHERE A.FTranType=21 and 1=(case when f.F_105 >0 and a.FCheckDate between f.f_106 and f.f_107 then 1 else 0 end)
					 AND A.FDate  BETWEEN @FDATE1 AND @FDATE2 AND A.FStatus =1
				*/	
		...
		left outer join (
			/*
			SELECT c.FItemID AS FEmpID,SUM(B.FConsignAmount) AS Amount_cnt3
			 FROM ICStockBill A
			INNER JOIN ICStockBillEntry B ON A.FInterID =B.FInterID 
			INNER JOIN t_Organization F ON F.FItemID = A.FSupplyID 
			INNER JOIN t_Emp C ON C.FItemID=f.F_105
			WHERE A.FTranType=21 and 1=(case when f.F_105 >0 and a.FCheckDate between f.f_106 and f.f_107 then 1 else 0 end)
			 AND A.FDate  BETWEEN @FDATE1 AND @FDATE2 AND A.FStatus =1
		...
		isnull(FAmount3,0) as famount3
			  ...
			  , (case when FXiShu=0 then case when (isnull(FAmount1,0)+isnull(FAmount2,0))>= (case  FZhiWuCd when 1045 then 350000 when 41470 then 350000 
																								   else 150000 end) 
											  then 2000 
											  else (isnull(FAmount1,0)+isnull(FAmount2,0))/(case  FZhiWuCd when 1045 then 350000 when 41470 then 350000
																								 else 150000 end)*2000 
											  end
					 else 0 end) as FSaleBaseAmount
			  , (case when (isnull(FAmount1,0)+isnull(FAmount2,0))<(case when FZhiWuCd = 41470	then 350000 when FZhiWuCd = 1045  then 350000   else 150000 end) then 0
					  else ((isnull(FAmount1,0)+isnull(FAmount2,0))-(case when FZhiWuCd = 41470  then 350000 when FZhiWuCd = 1045  then 350000 else 150000 end))*0.13
						 * (case when isnull(FPlanAmount,0)=0  then 0 
						         else	case	when (isnull(FAmount1,0)+isnull(FAmount2,0))/FPlanAmount >=0.9 then 1 
												when (isnull(FAmount1,0)+isnull(FAmount2,0))/FPlanAmount >=0.8 and (isnull(FAmount1,0)+isnull(FAmount2,0))/FPlanAmount<0.9 then 0.8
												when (isnull(FAmount1,0)+isnull(FAmount2,0))/FPlanAmount >=0.7 and (isnull(FAmount1,0)+isnull(FAmount2,0))/FPlanAmount<0.8 then 0.6
		...
		from(
			select (case when a1.fempid IN(256,200152) then 256 else a1.fempid end) as Femployee
				 , a0.FCheckAmount as FProceedsAmount,b.FNumber 
			 from t_RP_NewCheckInfo a 
			inner join t_Organization b on a.FCustomer=b.FItemID
			 join t_rp_newcheckinfo a0 on a.ftype = 5 and a0.ftype = 3 and a.fid = b.fid 
			 join icsale  a1 on a0.fbillid = a1.finterid
			inner join t_Emp c on c.FItemID=a1.fempid 
			INNER JOIN t_SubMessage E ON E.FParentID =29 AND E.FInterID =C.FDuty
			where  a.FCheckDate between @FDATE1 and @FDATE2
		...
		from(
			select (case when a1.fempid IN(256,200152) then 256 else a1.fempid end) as Femployee
				 , a0.FCheckAmount as FProceedsAmount
			 from t_RP_NewCheckInfo a 
			inner join t_Organization b on a.FCustomer=b.FItemID
			 join t_rp_newcheckinfo a0 on a.ftype = 5 and a0.ftype = 3 and a.fid = b.fid 
			 join icsale  a1 on a0.fbillid = a1.finterid
			...
			BEGIN
				...
				BEGIN
					...
					@FPlanFDate=(case	when b.FOptMode =0 and FFstStDate =1	--信用天数结算  且 起算日：单据月末日期
										...
										*/
			/* -----------------------------   REWRITE DATE:2017/02/07--WRITER: RenBwo ------the following------------------------------
			*/
			...
			/* -----------------------------   REWRITE DATE:2017/02/07--WRITER: RenBwo ------THE ABOVE-------------------- 
			*/
					else 0 end)
