...
;Select FUserID,FParentID,FLevel,FNumber,FState,FName FROM t_UserType WHERE FUserID>0  order by FLevel,FUserID
...
---login
;Select COUNT(FID) FROM t_UserProfile WHERE FUserID = 16394 AND fcategory = 'UserMgr' AND FKey = 'SelUserType' --Administrator
;Select * From t_user Where FUserID = 16394 ;--Administrator
;Select FValue From t_systemprofile Where fcategory='base' And FKey='InitDataLanguage'; --language
;Select FKey,FValue From t_SystemProfile Where fcategory='Base' And FKey IN ('Type','AcctType','UUID');--account base info
;Select FName,FUserID,FSafeMode,FSid,FForbidden,FSSOUsername From t_user Where FName= 'kingdee' And abs(FUserID) >= 16384;--login user info ,normal user id is after 16384
;Select top 1 * from t_systemprofile
;Select FUInValidDate,FPwCreateDate,FPwValidDay from t_User  where FUserID=16415 ; --username:kingdee
;Select count(1) as recordcount  from syscolumns where id=(Select id from sysobjects where name='t_log')--the numbers of columns about t_log
...
;Select FAuthRight from t_User  where FUserID=16415 --authority right
;Select FTimeStamp as lngTimeStamp From t_DataFlowTimeStamp Where FName = 'DataFlow'--contol console set
...
;Select  FSubSysID,FName as FName_CHS, FName_CHT, FName_EN,FName As FName,FTopClassID,FClassName,FIndex, FVisible,FShowSysType,FAcctType,FRefresh,FNote As FNote,FAppClassName,FHelpFileName, FTipID,F;updateBaseData,FSubID  From t_DataFlowSubSystem Order by FIndex,FSubSysID
...
;Select  FAccttype,FacctTrade,FName As FName,FDesc As FDesc,FShowName As FShowName,FNumber,FForbidden,F;DELETEd  From t_DataFlowAcctType Order by FAccttype
;Select * From t_DataFlowProfile;
;Select FTimeStamp  From t_DataFlowTimeStamp Where FName = 'DataFlow16415';
--selfdefine console
;Select t1.FTopClassID,t1.FTopClassName as FTopClassName_CHS, t1.FTopClassName_CHT, t1.FTopClassName_EN,t1.FTopClassName As FTopClassName
...
--CONTROL console end
 ...
 --1002510	工序汇报批量录入单
  ;Select FValue FROM ICClassUserProfile WHERE FUserID=16415 AND FSection='BillSet1002510' AND FKey='AutoInPhaseWPDefSelf'
  --icclasstype 单据定义表头	Doc Definition Header
 --icclasstableinfo 单据定义Doc Definition
 ;Select FTimeStamp FROM ICCLassType WHERE FID=1002510 --hex
...
--icclassmutex ?
;Select FDescription_chs as FDescription,*  FROM ICClassTypeEntry WHERE FParentID=1002510 ORDER BY FIndex 
;Select t1.FFuncID,t1.FOpareteType,t2.FNumber,t2.FSubSysID 
 FROM ICClassMutex t1 inner join t_SysFunction t2 on t1.FFuncID=t2.FFuncID --t_SysFunction 	系统功能表 
...
;Select * ,FName_CHS as FName,FDescriptions_CHS as FDescriptions FROM ICClassPredefineAction
;Select FUserID from t_Group where FUserID = 16415 and FGroupID = 1
;Select * FROM ICCLASSACTIONLIST T1 LEFT OUTER JOIN ICCLASSACTION T2 ON (T1.FCLASSACTIONID=T2.FID) WHERE T1.FCLASSTYPEID=1002510 ORDER BY T1.FORDER
...
;Select getdate() as 'Date'
;Select FUserID from t_Group where FUserID = 16415 and FGroupID = 1
;Select FValue,FKey FROM ICClassUserProfile WHERE FUserID=16415 AND FSection='BillSet1002510'
;Select FValue,FKey FROM ICClassUserProfile WHERE FUserID=16415 AND FSection='BillSet1002510'
;Select FValue,FKey FROM ICClassUserProfile WHERE FUserID=16415 AND FSection='BillSet1002510'
;Select FValue,FKey FROM ICClassUserProfile  WHERE FUserId = 16415 AND FSection = N'UserDefineOperation_1002510'
...
;Select FValue FROM t_SystemProfile WHERE FCategory='IC' AND FKey='RefreshAfterAdd'--增加后刷新
;Select FValue FROM t_SystemProfile WHERE FCategory='IC' AND FKey='BrID'--分支
;Select FValue FROM t_SystemProfile WHERE fcategory='StdCost' AND FKey='StdCostStart'--成本
;Select FUserID from t_Group where FUserID = 16415 and FGroupID = 1
;Select FUserID from t_Group where FUserID = 16415 and FGroupID = 1
 Drop Table #DATA 
 Drop Table #DATA1 
...
WHERE fcategory='IC' AND FKey IN ('StartBranchSale','ShowRelationSign','AuditChoice','PrecisionOfDiscountRate','StartPeriod','ListMaxRows','')
...
;Select FUserID from t_Group where FUserID = 16415 and FGroupID = 1
;Select FValue,FKey FROM ICClassUserProfile WHERE FUserID=16415 AND FSection='QUICKSEARCH_601'
;Select FValue FROM ICClassUserProfile WHERE FUserID=16415 AND FSection='QUICKSEARCH_601' AND FKey='UserLastProfile'
...
;Select FScale FROM t_Currency Where FCurrencyID=1
;Select FValue FROM t_SystemProfile WHERE fcategory='IC' AND FKey='SInvoiceDecimal'
;Select top 50000 u1.FAutoTD as FAutoTDID,u1.FEntryID as FEntryID_Number,u1.FWBInterID as FBillInterID,u1.FWorkBillNo as FBillNo,u1.FStatus as FStatus,v1.FTranType as FTranType,u1.FInterID as FInterID,u1.FEntryID as FEntryID,v1.FDate as FDate,v1.FICMOInterID as FICMOInterID, 0 As FBOSCloseFlag 
from SHWorkBill v1 INNER JOIN SHWorkBillEntry u1 ON   v1.FInterID = u1.FInterID  AND u1.FInterID<>0 
...
;Select FValue FROM ICClassUserProfile WHERE FUserID=16415 AND FSection='List601' AND FKey='DisplayQuickSearch'
;Select FValue FROM ICClassUserProfile WHERE FUserID=16415 AND FSection='List601' AND FKey='SearchInScheme'
;Select FValue FROM ICClassUserProfile WHERE FUserID=16415 AND FSection='List601' AND FKey='ViewUnionQuery'
;Select FValue FROM ICClassUserProfile WHERE FUserID=16415 AND FSection='List601' AND FKey='UnionQueryCtlHeight'
...
;Select  FTypeID,FInterID FItemID, FID FNumber,FName,FSpec FROM t_SubMessage Where FInterID>0 AND F;DELETEd=0  And FTypeID=62 ORDER BY FID
;Select top 40 
...
;Select ISNULL(FAuxQtyPlan - FAuxQtyFinish ,0) as FAuxQty,FWBInterID  FROM SHWorkBillEntry  
 WHERE 1=2  OR FWBInterID =745738 OR FWBInterID =0
;Select * from ICClassLink where FSourClassTypeID = 1002510
...
update  ICClassUserProfile SET FValue='mnu;SelectOldBill_52' WHERE FUserID=16415
 AND FSection='BillSet1002510' AND FKey='SelBill' ELSE 
INSERT INTO ICClassUserProfile(FUserID,FSection,FKey,FValue )  
  VALUES(16415,'BillSet1002510','SelBill','mnu;SelectOldBill_52')
...
;Select T1.FKey,T1.FValue  FROM ICSHOP_SCProfile T1
...
VALUES(0,'',1,745738,'WB081657',380199,'SC1503-562', '2017-01-09 08:55:47',22,0, 11,11,11  )
...
;Select T1.*  FROM #TEMP_RuleCheck T1
...
;INSERT INTO #TEMP_RuleCheck(FRptInterID,FRptBillNo,FEntryID,FWBInterID,FWBBillNo,FICMOInterID,FICMOBillNo
                           ,FRptBeginDate,FRptSumAllZero,FRptTimeAllZero
                           ,FRptAuxQtyFinishALLRow,FRptAuxQtyFinish,FRptSumQtyOther)
VALUES(0,'',1,745738,'WB081657',380199,'SC1503-562','2017-01-09 08:55:47',6,0,  3,3,3  )
...
;Select 0 AS FRptInterID,'1' AS FRptBillNo,3 AS FSumAuxQtyPass,T2.FinterID,T2.FWBInterID,T2.FOperSN,T2.FOperID,T3.FID AS FNumber,T3.FName
  FROM SHWorkBillEntry T2 INNER JOIN t_SubMessage T3 ON T2.FOperID = T3.FInterID AND T3.FTypeID = 61
 WHERE T2.FQualityChkID = 352 AND T3.FInterID>0 AND T3.F;DELETEd=0 AND T2.FWBInterID = 745738
...
;Select t1.FSourClassTypeID as FClassID ,t2.FName_CHS as FName,  FAllowCopy,FAllowCheck,FAllowForceCheck, t1.FRemark, t1.FFlowControl,t1.FUseSpec 
 ...
 FROM ICClassLink t1 left join ICClassType t2 on t1.FSourClassTypeID=t2.FID
 Left Join ICClassTableInfo t3 on t1.FDestClassTypeID = t3.FClassTypeID and t1.FSRCIDKey = t3.FKey
 where t1.FDestClassTypeID=1002510 and t1.FIsUsed=2 And t1.FSourClassTypeID 
 in (Select FSourClassTypeID from ICClassLinkEntry where FDestClassTypeID = 1002510) AND t1.FUnControl & 2 = 2
...
;Select fcategory,FKey,FValue FROM t_SystemProfile WHERE fcategory='IC' AND FKey IN ('StartBranchSale','ShowRelationSign','AuditChoice','PrecisionOfDiscountRate',
'StartPeriod','ListMaxRows','')
;Select FTemplateID,FName As FName ,FFilter from ICListTemplate Where FID=601
;Select FID,FName As FName ,FTemplateID,FFontID,FLogicStr,FToolBarVis,FHeadVis,FBottomVis,FSourceType,FSourceSql,FGroupID,FBillTemplateID,FNeedCount,FHeadHeight,FBottomHeight,FType,FGetDataType,FMenuID,FBillCls,FFilter,FRptTemplateID,FMasterTable,FNeedStatistic From ICListtemplate  Where FID in (601)

;Select FInterID,FTypeID,FColCaption as FColCaption_CHS,FColCaption As FColCaption ,FHeadSecond,FColName,FTableName,FColType, FColWidth,FVisible,FItemClassID,FVisForQuest,FReturnDataType,FCountPriceType, FCtlIndex,Case When FColName = 'FSourceTranType' Then 'FName' Else FName End As FName,FTableAlias,FAction,FNeedCount,FIsPrimary,FLogicAction,FStatistical,FMergeable,FVisForOrder,FControl,FMode,FControlType,FEditable,FFormat,FFormatType,FAlign,FMust;Selected
FROM ICChatBillTitle Where FTypeID IN (Select FTemplateID From ICListtemplate  Where FID in (601)) order by FInterID
...
;Select 0 AS FRptInterID,'1' AS FRptBillNo,2 AS FSumAuxQtyPass,T2.FinterID,T2.FWBInterID,T2.FOperSN,T2.FOperID,T3.FID AS FNumber,T3.FName
  FROM SHWorkBillEntry T2 INNER JOIN t_SubMessage T3 ON T2.FOperID = T3.FInterID AND T3.FTypeID = 61
 WHERE T2.FQualityChkID = 352 AND T3.FInterID>0 AND T3.F;DELETEd=0 AND T2.FWBInterID = 2604765
...
;Select FStatus From ICSchemeProfile WHERE FSysName='(601)' AND FUserID=16415 AND FStatus in (1,2) AND FListShowStatus = 2
;INSERT INTO ICSchemeProfile(  FSchemeID , FSchemeName , FUserID ,  FTranType , FSysName , FStatus , FListShowStatus  ) 
VALUES(6412,'默认方案',16415,601,'(601)',2 , 2)
...
;Select * FROM ICListFilter WHERE FListID = 601 And FListShowStatus = 2 AND FDestBillTranType=1002510 AND FBillROB=0 AND FCheckSwitch=0
...
;Select FValue FROM t_SystemProfile WHERE fcategory='IC' AND FKey='StartBranchSale'
;Select FValue FROM t_SystemProfile WHERE fcategory='IC' AND FKey='ShowRelationSign'
;Select t_User.FUserID from t_Group, t_User  where t_Group.FUserID = t_User.FUserID  And t_User.FUserID = 16415 and t_Group.FGroupID = 1 
;Select FValue From t_Systemprofile Where fcategory = 'IC' and FKey='CheckUserGroup' 
;Select * FROM ICTableRelation WHERE FTypeID=601 ORDER BY FInterID,FTableNameAlias,FFieldName,FTableNameAlias11,FFieldName11,FIndex
;Select * FROM ICTableRelation WHERE FTypeID=601 ORDER BY FInterID,FTableNameAlias,FFieldName,FTableNameAlias11,FFieldName11,FIndex
;Select * FROM ICTableRelation WHERE FTypeID=601 ORDER BY FInterID,FTableNameAlias,FFieldName,FTableNameAlias11,FFieldName11,FIndex
...
;Select top 50000 u1.FAutoTD as FAutoTDID,u1.FEntryID as FEntryID_Number,u1.FWBInterID as FBillInterID,u1.FWorkBillNo as FBillNo,u1.FStatus as FStatus,v1.FTranType as FTranType,u1.FInterID as FInterID,u1.FEntryID as FEntryID,v1.FDate as FDate,v1.FICMOInterID as FICMOInterID, 0 As FBOSCloseFlag from SHWorkBill v1 INNER JOIN SHWorkBillEntry u1 ON   v1.FInterID = u1.FInterID  AND u1.FInterID<>0 
 ...
 LEFT OUTER JOIN t_subMessage t91 ON   u1.FAutoTD = t91.FInterID  AND t91.FInterID<>0 
 LEFT OUTER JOIN t_subMessage t92 ON   u1.FAutoOF = t92.FInterID  AND t92.FInterID<>0 
 where 1=1 AND ( u1.FPlanStartDate >=  '2016-08-01' )  AND (( u1.FStatus=1 and u1.FAutoTD=1058 
 and (Select fstatus from icmo where finterid=v1.ficmointerid) in(1,2) and 
 (Select fsuspend from icmo where finterid=v1.ficmointerid) =0  ) AND 
 (((u1.FStatus=0 OR u1.FStatus=1 OR u1.FStatus=2 OR u1.FStatus=3))  AND 
 (v1.FDate>='2016-06-01' AND  v1.FDate<'2016-07-01'))) order by  u1.FInterID,u1.FEntryID
 ;Select ISNULL(FValue,0) AS FValue FROM t_SystemProfile WHERE fcategory='IC' AND FKey='ListMaxRows'
...
;Select 0 AS FRptInterID,'1' AS FRptBillNo,30 AS FSumAuxQtyPass,T2.FinterID,T2.FWBInterID,T2.FOperSN,T2.FOperID,T3.FID AS FNumber,T3.FName
  FROM SHWorkBillEntry T2 INNER JOIN t_SubMessage T3 ON T2.FOperID = T3.FInterID AND T3.FTypeID = 61
 WHERE T2.FQualityChkID = 352 AND T3.FInterID>0 AND T3.F;DELETEd=0 AND T2.FWBInterID = 4754980
...
--更新流转号中间表SHMoveNoManage
 ...
 EXEC prc_SH;updateBillDoubleQty -502, @FTaskDispID
 EXEC prc_SH;updateBillDoubleQty -52, @FWBInterID
 EXEC prc_SH;updateBillDoubleQty 582, @FInterID
