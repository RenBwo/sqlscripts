...
--#ParentData
 ...
 --- GetBomOpen 展开嵌套SQL 开始 by wwq 版本v90 -------
	...
	INNER JOIN icbomChild_201506061535 V2  ON V1.FInterID=V2.FInterID       
	...
	left join t_Department t2 on T1.FSource=t2.FItemID and T1.FErpClsID  in (2,7,9)   
	 ...
	 and (((v2.FMaterielType=371 or v2.FMaterielType=375)))
				 and u1.FIsCharSourceItem = 0 And u1.FCharConfigID = 0 
				 end    --过滤掉特性来源物料和特性物料  
select * from  #BomData
/*
		 --- From ICCustChild
...
-------------------------------------特性配置处理开始------------------------------------------
	...
	select * from  #TempChar 
	select * from  #TempBomOpen_Char 
	...
	=(SELECT top 1 CONVERT(VARCHAR,vv2.FItemId ) + '|' + CONVERT(VARCHAR,vv2.fCheckdate,111) + '|'+ CONVERT(VARCHAR,vv2.finterid) +'|' + CONVERT(VARCHAR,vv2.fentryid) 
	   ...
	   where exists       (Select v2.FItemID 
			 ...
			 where v1.FTranType in (1,2,5,10) and convert(char(8),v1.Fdate,112)<=getdate()
 ...
 where (substring(c.FName,len(c.FName)-1,2))='90' AND (A.FModel LIKE '%纸箱%' OR A.FModel LIKE '%包装箱%') 
 )
	 ...
	 Left Join (
		Select top 1 v2.FItemID 
		  ...
		  where v1.FTranType in (1,2,5,10) and v2.FItemID =41264 and v1.FStatus > 0 
		   and convert(char(8),v1.Fdate,112)<=getdate()  order by v1.fdate desc,v1.FInterID desc
